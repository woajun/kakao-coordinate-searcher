...
export function createFuzzyMatcher(input: string) {
  const pattern = input.toLocaleLowerCase().split('').map(ch2pattern).join('.*?');
  return new RegExp(pattern);
}

export default function search<T>(query: string, list: T[]): T[] {
  const regex = createFuzzyMatcher(query);
  return list.filter((e: any) => regex.test(e.toLocaleLowerCase()));
}
